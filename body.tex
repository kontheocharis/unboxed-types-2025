...
\]
annotated with the size of its captures $k$. Unfortunately, this is not
...
This also means it is not immediately compatible with 2LTT. We leave investigating this
as future work, but it works least in the absence of staging.
...
irrelevance without further modifying the structure of contexts. We can do this through
a monadic modality
\[
...
for the construction of runtime-sized data such as pairs. Boxing can now be relaxed
to operate on runtime-sized data, which in particular enables the construction of runtime-polymorphic
functions such as $\syn{id} : (A : \univ_b) \to \square A \to \square A$.
...
currently working on a proof-of-concept implementation of this system which is backed by
LLVM.
